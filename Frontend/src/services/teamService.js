...
/**
 * Récupère les informations sur le coach assigné à une équipe.
 * @param {string} teamId - L'ID de l'équipe.
 * @returns {Promise<object>} - Les informations du coach.
 */
const getCoach = async (teamId) => {
    try {
        const token = localStorage.getItem('token');
        const response = await axios.get(`${API_URL}/${teamId}/coach`, {
            headers: { Authorization: `Bearer ${token}` },
        });
        return response.data;
    } catch (err) {
